...
public class PlayerAction : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        bool vUp = manager.isAction ? false : Input.GetButtonUp("Vertical") || up_Up || up_Up;

        //Check Horizontal Move
        if (hDown)//▷ 수평을 누르거나 수직에서 손을 뗐을 경우
            isHorizonMove = true;
        else if (vDown)//▽
            isHorizonMove = false;
        else if (hUp || vUp)
    ...
    void FixedUpdate()
    {
        ...
        RaycastHit2D rayHit = Physics2D.Raycast(rigid.position, dirVec, 0.7f, LayerMask.GetMask("Object"));//Object만 스캔하게 됨
